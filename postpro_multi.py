...
# writing the multiprocessing script
import multiprocessing
from multiprocessing import Pool
from functools import partial
import os

def file_processor(out_dir):
  try:
...
# post processing by calling the functions
num_workers = multiprocessing.cpu_count()
pool = Pool(num_workers)

# with tqdm, and save in results
results = list(tqdm(pool.imap(partial(file_processor), files), total=len(files)))
...
# combine and save the files from the results
for i, data in enumerate(results):
  save_dir = Path(args.par_dir) / 'pt_data'
  torch.save(data, save_dir / f'freefall_{i}.pt')
  print(f'freefall_{i}.pt saved')
